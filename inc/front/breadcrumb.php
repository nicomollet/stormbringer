 ...
 */
function stormbringer_breadcrumb( $args = array() ) {
	/* Get the textdomain. */
...
function breadcrumb_trail_get_items( $args = array() ) {
	...
	elseif ( is_singular() ) {
      ...
      /* If $front has been set, add it to the $path. */
			...
			//if ( !empty( $path ) ) // fix pour talenco blog
				//$trail = array_merge( $trail, breadcrumb_trail_get_parents( '', $path ) );// fix pour talenco blog
      ...
      foreach ($paths as $path_item) {
        if ($path_item != '') {
          foreach (get_post_types() as $post_type_archive) {
            if ($post_type_archive != $post_type) {
              $post_type_object_archive = get_post_type_object($post_type_archive);
      ...
      /* If there's a path, check for parents. */
      if ( !empty( $path ) )
      $trail = array_merge( $trail, breadcrumb_trail_get_parents( $parent, $path ) ); // Ajout $parent 07/09/2012
