	...
	elseif ( is_singular() ) {
		...
		elseif ( 'page' !== $post_type ) {
			...
			if ( !empty( $post_type_object->has_archive ) )
				$trail[] = '<a href="' . get_post_type_archive_link( $post_type ) . '">' . $post_type_object->labels->name . '</a>';
		}
...
function breadcrumb_trail_map_rewrite_tags( $post_id = '', $path = '', $args = array() ) {
	...
	if ( is_array( $matches ) ) {
		...
		foreach ( $matches as $match ) {
			...
			elseif ( '%author%' == $tag )
				$trail[] = '<a href="' . get_author_posts_url( $post->post_author ) . '" >' . get_the_author_meta( 'display_name', $post->post_author ) . '</a>';
			...
			elseif ( '%category%' == $tag && 'category' !== $args["singular_{$post->post_type}_taxonomy"] ) {
				...
				if ( $terms ) {
					...
					$trail[] = '<a href="' . get_term_link( $term, 'category' ) . '" >' . $term->name . '</a>';
				}
			}
		}
	}
